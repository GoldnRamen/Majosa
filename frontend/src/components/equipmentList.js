      ...
      {selectedEquip === null ? (
        <section className='mt-10 lg:mt-5 p-2'>          
          ...
          <div className="text-black rounded-xl shadow-2xl w-full h-fit relative animate-in fade-in zoom-in duration-300 bg-gray-400">            
            ...
            >
              <MdOutlineClose className="text-indigo-950 hover:text-red-900 text-xl" onClick={() => {setSelectedEquip(null); setCloseBtn(true)}} />        
            </button>            
            {selectedEquip && (
            <div className='z-20 relative'>
              <div className='lg:grid p-2 lg:grid-cols-3 lg:gap-2'>  
                <h2 className="flex lg:justify-center lg:hidden font-bold text-lg my-5 px-3 text-cyan-900">{selectedEquip.product}</h2>
                <div className='col-span-1'>
                  <video className="w-full h-full object-cover lg:rounded-l-lg" autoPlay loop muted playsInline>
                    <source src={selectedEquip.img} type="video/mp4" />
                  </video>
                </div>
                <div className='col-span-2 space-y-5 lg:p-6'>
                  <h2 className="lg:flex justify-center hidden font-bold text-lg my-5 text-cyan-900">{selectedEquip.product}</h2>
                  <p className='lg:mx-10 m-5'>{selectedEquip.desc}</p>
                </div>
              </div>
            </div>
            )}
