...
export default function CompaniesGrid(){
  ...
  const handleCardClick = (id) => {
    if (router.pathname !== companyDetailPage) {
      router.push({
        pathname: companyDetailPage,
        query: { select: id },
      });
      return;
    }
  ...
  return (
    ...
    {selectedId === null ? (
      <section id="companies" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {companies.map(c => (
          <div key={c.id} className={`group relative border overflow-hidden rounded-2xl shadow shadow-black ${c.bgColor} border-5 shadow-lg`}>
            ...
            <div className="absolute inset-x-0 lg:bottom-0 bottom-15 translate-y-full lg:group-hover:translate-y-0 transition-all duration-500 backdrop-blur-md bg-white/20 p-4">              
